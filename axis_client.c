...
subscription_callback(guint subscription, AXEvent *event, guint client_socket)
{
  ...
  if (active)
  {
    syslog(LOG_INFO, "High motion detected");
    g_message("High motion detected: %s", seconds;
  }
  else
  {
    syslog(LOG_INFO, "Low motion detected");
    g_message("Low motion detected: %s", seconds;
  }
